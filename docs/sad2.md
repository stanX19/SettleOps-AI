...
|---|---|---|---|
| 0.1 | 2026-04-23 | Team | Initial draft — full system specification for hackathon build |
...
flowchart TD
    subgraph Browser["Browser (Next.js)"]
        FileClaim["/file-claim<br/>Claimant upload view"]
        Dashboard["/dashboard<br/>Operator 3-pane dashboard"]
    end

    subgraph Backend["FastAPI Backend (Python)"]
        Http["HTTP Endpoints<br/>/submit-claim<br/>/case/{id}<br/>/case/{id}/pdf"]
        Stream["SSE Streamer<br/>/case/{id}/stream"]
        Store["In-Memory Case Store<br/>CaseState by case ID"]
        Runtime["LangGraph Agent Runtime<br/>Intake -> Policy/Liability/Fraud -> Payout -> Auditor"]
        Pdf["PDF Renderer<br/>(ReportLab)"]
    end

    subgraph External["External Services"]
        GLM["GLM API<br/>Text + vision inference"]
    end

    FileClaim -- "HTTP multipart upload" --> Http
    Dashboard -- "SSE subscription" --> Stream
    Http --> Store
    Stream --> Store
    Store --> Runtime
    Runtime --> Store
    Runtime -- "text + vision calls" --> GLM
    Runtime --> Pdf
    Pdf --> Store
    Stream -- "progress events" --> Dashboard
    Http -- "case data and artifacts" --> Dashboard
```
...
stateDiagram-v2
    ...
    ParallelFork --> Policy
    ParallelFork --> Liability
    ParallelFork --> Fraud

    state ParallelJoin <<join>>
    Policy --> ParallelJoin
    Liability --> ParallelJoin
    Fraud --> ParallelJoin
    ParallelJoin --> Payout

    Payout --> Auditor
    Auditor --> GeneratePDF: approve
    Auditor --> Escalated: escalate or loop_count >= 2
    Auditor --> Policy: challenge policy / loop_count < 2
    Auditor --> Liability: challenge liability / loop_count < 2
    Auditor --> Fraud: challenge fraud / loop_count < 2
    Auditor --> Payout: challenge payout / loop_count < 2

    Policy --> Payout: re-run complete
    Liability --> Payout: re-run complete
    Fraud --> Payout: re-run complete

    GeneratePDF --> AwaitingApproval
    AwaitingApproval --> [*]
    Escalated --> [*]
```
...
| Level | Scope | Runner | When to run |
|---|---|---|---|
| Unit tests | Individual Pydantic contracts, arithmetic helpers, PDF extraction utilities | `pytest` | On every commit and before daily integration checkpoint |
| Agent tests | Each agent in isolation with mocked GLM responses | `pytest` + `httpx` mock | After modifying any agent prompt or contract |
| Integration tests | Full LangGraph pipeline end-to-end with a sample case | `pytest` + real GLM calls | Once per day minimum; before every demo rehearsal |
| Manual UI tests | Frontend upload flow, SSE rendering, dashboard interaction | Browser (Chrome DevTools) | After any frontend change; full walkthrough on day 5–6 |

### 15.2 Unit tests (`backend/tests/test_contracts.py`)

Validate all Pydantic models with edge cases:

- **CaseFacts:** missing optional fields (no chat, no coordinates), narrative at exactly 500 words, extraction_confidence at boundary values (0.0, 1.0)
- **PolicyVerdict:** zero excess, zero NCD, empty exclusions list, depreciation schedule with multiple brackets
- **LiabilityVerdict:** fault percentages summing to exactly 100, `is_valid()` returns false when sum != 100
- **FraudAssessment:** suspicion_score at 0.0 (clean) and 1.0 (max), empty signals list
- **PayoutRecommendation:** all four recommended_action values, final_payout_myr = 0 edge case
- **AuditResult:** approve with empty challenges, challenge with multiple agents, escalate verdict

Also test:
- Payout arithmetic helpers (`estimate_from_damage_list`, `apply_depreciation`) with known inputs and expected outputs
- PDF extraction pipeline: pdfplumber → pypdf → OCR fallback chain with a sample PDF
- GLM client retry logic with simulated 429 and timeout responses

### 15.3 Agent tests (`backend/tests/test_agents.py`)

Each agent is tested with a mocked GLM client that returns pre-recorded responses:

1. **Fixture setup:** a complete `CaseState` with realistic sample data
2. **Happy path per agent:** feed valid inputs, assert output matches the expected Pydantic contract
3. **Malformed GLM response:** simulate a response that fails Pydantic validation, verify the retry-once-then-fail logic triggers
4. **Timeout handling:** simulate a 15s timeout, verify graceful failure and status update

Mock strategy: replace the GLM HTTP client with a `pytest` fixture that returns canned JSON. Do not call the real GLM API in agent tests — save quota for integration runs.

### 15.4 Integration tests

Run the full `compiled.ainvoke(state)` pipeline against the real GLM API with the sample case from `demo-assets/`:

- **Happy path:** verify workflow completes with status `awaiting_approval` and a PDF is generated
- **Auditor challenge loop:** use a crafted case with a deliberate inconsistency between photo damage and narrative; verify the auditor challenges, the loop count increments, and the pipeline eventually converges or escalates
- **Fraud escalation:** use a case where policy inception is < 30 days before incident; verify `suspicion_score > 0.6` and status becomes `escalated`

Integration tests are expensive (GLM quota, ~60s per run). Run them deliberately, not on every commit.

### 15.5 Frontend manual test checklist

Run through these before every demo rehearsal:

- [ ] `/file-claim`: upload police report PDF, policy PDF, 3 photos, paste chat transcript — submission returns a case ID
- [ ] `/file-claim`: submit with missing optional chat transcript — still succeeds
- [ ] `/file-claim`: submit with no photos — still succeeds (Liability agent handles gracefully)
- [ ] `/dashboard`: SSE connection establishes and agent nodes begin animating
- [ ] `/dashboard`: all 6 agent nodes transition through started → completed states
- [ ] `/dashboard`: right pane Blackboard updates with each agent's output JSON
- [ ] `/dashboard`: if auditor challenges, backward arrow animates and challenged node pulses orange
- [ ] `/dashboard`: "Approve" button appears after workflow completes, clicking it updates status
- [ ] `/dashboard`: "Override" button accepts rationale text and regenerates PDF
- [ ] `/dashboard`: PDF preview loads and download link works
- [ ] Browser refresh during workflow: SSE reconnects and state is recovered

### 15.6 Demo rehearsal protocol

Starting day 5, run the full demo sequence at least twice daily:

1. Open `/file-claim` on a phone (or phone-width browser)
2. Upload the sample case from `demo-assets/`
3. Switch to `/dashboard` on the projector display
4. Watch the full pipeline execute
5. Click Approve, download the PDF
6. If any step fails, log the failure and fix before the next rehearsal
7. If the live run stalls > 20s, test the cached replay fallback (Section 9.3)
