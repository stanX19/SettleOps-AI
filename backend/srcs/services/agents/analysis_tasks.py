...
def _build_available_files_block(state: StateLike) -> str:
...
def build_citation_instruction(state: StateLike, node_id: str) -> str:
    ...
    """
    available = _build_available_files_block(state)
    return f"""
CITATION REQUIREMENT (MANDATORY):
Every output field in "data" must be backed by at least one citation.
Do NOT guess filenames — only use the exact stored filenames listed below.
